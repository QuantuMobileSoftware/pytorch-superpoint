...
from common import SatSettings
    ...
    for skysat_path, skysat_bound in zip(skysat_paths, skysat_bounds):
        for other_path, other_bound in zip(other_paths, other_bounds):
            if skysat_bound.intersects(other_bound):
                pairs.append((skysat_path, other_path))

    for skysat_path, other_path in tqdm(pairs):
        ...
        for num, mw in enumerate(windows_skysat):
            ...
            write_image(SatSettings.compressed_skysat_dir/f"{stack_name}_{num}.jpg", skysat_crop)
            write_image(SatSettings.compressed_other_dir/f"{stack_name}_{num}.jpg", other_crop)
